...
resource "aws_security_group" "statico-batch" {
  name = "aws-batch-statico-security-group"
...
resource "aws_batch_compute_environment" "statico" {
  compute_environment_name = "statico-sample"
  compute_resources {
    ...
    min_vcpus = 0
    security_group_ids = [
      "${aws_security_group.statico-batch.id}"
    ]
    subnets = [
...
resource "aws_batch_job_queue" "statico" {
  name = "statico-queue"
  state = "ENABLED"
  priority = 1
  compute_environments = [ 
    "${aws_batch_compute_environment.statico.arn}"
  ]
}

resource "aws_iam_role" "job-role" {
  name = "aws-batch-statico-job-role"
  path = "/BatchSample/"
  assume_role_policy = <<EOF
{
    "Version": "2012-10-17",
...
resource "aws_batch_job_definition" "statico-job" {
  name = "statico-job"
  type = "container"
...
### Dynamodb
resource "aws_dynamodb_table" "statico_config" {
  name         = "statico_config"
